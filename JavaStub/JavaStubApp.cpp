...
int bit = 0;
bool fsortInfo(JavaInfo i, JavaInfo j) { 
	return (i < j); 
}
...
void JavaStubApp::scanForJavaVM()
{
	...
	// sorta le jdk in ordine di versione: la più recente sarà l'ultima
	if (!jdkList.empty())
	{
		std::sort(jdkList.begin(), jdkList.end(), fsortInfo);

		// se non è stata specificata una versione java da usare prende quella più recente tra quelle trovate
		const JavaInfo& ji = jdkList.back();
		javaHome = ji.path;

		if(!JVMVersion.empty())
		{
			// dichiarazione di una versione esplicita
			// supporta sia il formato 1.x che il formato più recente x (es. 8)
			// se contiene '+' (es. 1.8+) prende la versione specificata come minima e quella più recente tra quelle trovate come massima

			int minVersion, maxVersion;
			if(JVMVersion.find(L"1.") == 0)
			{
				minVersion = _wtoi(JVMVersion.substr(2).c_str());
				maxVersion = minVersion;
			}
			else
			{
				minVersion = _wtoi(JVMVersion.c_str());
				maxVersion = minVersion;
			}

			if (JVMVersion.find(L"+") != QString::npos)
			{
				maxVersion = ji.v[0];
			}

			LOG(1, L"Versione esplicita di jvm (JVMVersion=%s) min=%d max=%d\n", JVMVersion.c_str(), minVersion, maxVersion);

			for(const JavaInfo& j: jdkList)
			{
				if (j.v[0] < minVersion)
					continue;
				if (j.v[0] > maxVersion)
...
{
    for (const WCHAR** p = ignoreJava; *p != NULL; ++p)
        if (toIgnore.find(*p) != QString::npos)
            return true;
